...
import {
  ...
  type ItemFilters,
  deleteItem,
  getInsumosNoAsignados,
  asignarInsumoAEquipo,
  eliminarAsignacion,
  getEstados,
...
export default function ItemList({
  ...
  const [selectedEquipoDetail, setSelectedEquipoDetail] = useState<
    string | null
  >(null);
  ...
  function isEquipo(item: Item): item is Equipo {
    return (
      (item as Equipo).numero_serie !== undefined &&
      (item as Equipo).numero_serie !== null
    );
  }
    ...
    <div className="table-responsive rounded shadow p-3 mt-4">
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            {selectedItemImage?.name || "Imagen del item"}
          </Modal.Title>
        </Modal.Header>
      ...
      >
        ...
        >
          <Modal.Title>
            Asignar insumo a {selectedEquipo?.modelo?.nombre ?? "Equipo"} (
            {selectedEquipo?.numero_serie ?? "N/A"})
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {loadingInsumos ? (
            ...
            <>
              ...
              {selectedEquipo &&
                ...
                <ul className="list-group mb-3">
                  {selectedEquipo.asignaciones.map((insumo) => (
                    ...
                    >
                      <div>
                        <strong>{insumo.modelo ?? "N/A"}</strong> (
                        {insumo.marca ?? "N/A"})
                      </div>
                      <Button
              ...
              <Form.Select
                ...
                onChange={(e) =>
                  setInsumoSeleccionadoId(Number(e.target.value))
                }
              >
                <option value="">Selecciona un insumo</option>
                {insumosDisponibles.map((insumoAgrupado) => (
                  <option
                    key={insumoAgrupado.modelo_id}
                    value={insumoAgrupado.modelo_id}
                  >
                    {insumoAgrupado.modelo?.nombre ||
                      `Insumo #${insumoAgrupado.modelo_id}`}{" "}
                    ({insumoAgrupado.cantidad} disponibles)
                  </option>
                ))}
              </Form.Select>
            </>
        ...
        <Modal.Footer>
          <Button
            variant="secondary"
            onClick={() => setShowAccesorioModal(false)}
          >
            Cancelar
          </Button>
          <Button
      ...
      <Modal
        ...
        onHide={() => setShowAsignacionesModal(false)}
        size="lg"
      >
        <Modal.Header
          className="text-white py-3"
              ...
              <tbody>
                {currentAsignaciones.map((asignacion) => (
                  <tr key={asignacion.id}>
                    ...
                    <td>
                      <Badge
                        bg={asignacion.tipo === "equipo" ? "primary" : "info"}
                      >
                        {asignacion.tipo}
                      </Badge>
                    </td>
