...
const ReporteUsoAulas = () => {
  ...
  useEffect(() => {
    const fetchUsuarios = async () => {
      try {
        ...
        toast.error("Error cargando usuarios", { id: "error-cargar-usuarios" });
      }
    };
  ...
  const fetchReservas = async (page = 1) => {
    ...
    if (!fechaInicio || !fechaFin || !usuarioId) {
      toast.error("Selecciona usuario y ambas fechas", { id: ERROR_FILTROS_ID });
      return;
    }
                ...
                await new Promise<void>((resolve, reject) => {
                  logo.onload = () => {
                    try {
                      ...
                      const usuarioNombre = usuarioSeleccionado
            ...
            <Col md={3}>
              <Form.Group controlId="usuarioId">
                ...
                <label className="form-label d-flex align-items-center">
                  Seleccionar Usuario <span className="text-danger">*</span>
                </label>
                <Select
                  ...
                  value={
                    usuarioId
                      ? {
                        ...
                        label: usuarios.find((u) => u.id === usuarioId)
                          ? `${usuarios.find((u) => u.id === usuarioId)?.first_name} ${usuarios.find((u) => u.id === usuarioId)?.last_name}`
                          : "",
                      }
                      : null
                  }
                  onChange={(selected) => setUsuarioId(selected ? parseInt(selected.value) : null)}
